    ...
    public class RemoteManagableViewDefinitionRepositoryTests : TestWithContextBase
    {
        ...
        public void ViewHasRightValue()
        {
            ...
            using (var remoteClient = Context.CreateUserClient())
            {
                ...
                {
                    var viewComputationResultModel = remoteViewClient.GetResults(vd.Name, ExecutionOptions.Live).First();
            ...
            public void RoundTrippedViewsInit(ViewDefinition viewDefinition)
            {
                using (var remoteClient = Context.CreateUserClient())
                {
                    ...
                    try
                    {
                        ...
                        using (var remoteViewClient = Context.ViewProcessor.CreateClient())
                        {
                            Assert.NotNull(remoteViewClient.GetResults(viewDefinition.Name, ExecutionOptions.Live).First());
                        }
                    }
                    finally
                    {
