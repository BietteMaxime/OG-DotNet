...
namespace OpenGamma.Utils.Tuple
{
    public class PairTests
    {
        ...
        private void CheckRoundTripImpl<TFirst, TSecond>(Pair<TFirst, TSecond> orig, bool skipNonGenericCheck = false)
        {
            ...
            {
                var nonGenericMessage = new global::Fudge.FudgeMsg(_fudgeContext, message.GetAllFields().Where(f => f.Ordinal != 0 || !((string)f.Value).Contains('`')).ToArray());
                CheckRoundTrip(fudgeSerializer, orig, nonGenericMessage);
            }
        }

        private static void CheckRoundTrip<TFirst, TSecond>(FudgeSerializer fudgeSerializer, Pair<TFirst, TSecond> orig, global::Fudge.FudgeMsg message)
        {
