...
{
    public class EnumBuilder<T> : // NOTE: we don't implement BuilderBase<T> because then boxing would bite us
        BuilderBase<object>
        where T : struct
    {
        #region static utils
        ...
        private static string GetJavaNameImpl(T value)
        {
            ...
            if (field != null)
            {
                object[] attributes = field.GetCustomAttributes(typeof(FudgeValueAttribute), false);
                if (attributes.Length == 1)
                {
                    return ((FudgeValueAttribute) attributes[0]).Value;
                }
            }
        ...
        protected override object DeserializeImpl(IFudgeFieldContainer msg, IFudgeDeserializer deserializer)
        {
