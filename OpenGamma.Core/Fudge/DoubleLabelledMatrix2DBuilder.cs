...
namespace OpenGamma.Fudge
{
    class DoubleLabelledMatrix2DBuilder : BuilderBase<DoubleLabelledMatrix2D>
    {
        ...
        protected override DoubleLabelledMatrix2D DeserializeImpl(IFudgeFieldContainer msg, IFudgeDeserializer deserializer)
        {
            string xTitle = msg.GetString("xTitle");
            string yTitle = msg.GetString("yTitle");
            string valuesTitle = msg.GetString("valuesTitle");

            IFudgeFieldContainer matrixMsg = msg.GetMessage(MatrixField);
            ...
            bool newRow = true;
            int count = -1;
            foreach (var field in matrixMsg)
            {
