...
{
    public class RemoteViewClientBatchTests : RemoteEngineContextTestBase
    {
        ...
        public void CanRunSingleCycleBatchParallel()
        {
            ...
            Parallel.For(1, 20, new ParallelOptions { MaxDegreeOfParallelism = 4 }, delegate(int i)
            {
                ...
                AssertApproximatelyEqual(req.ExecutionSequence.Next.ValuationTime, 
                                         runToCompletion.Item2.Single().FullResult.ValuationTime);
            });
        }
        ...
        public void CanRunSingleYesterdayCycleBatchWithVersion()
        {
            ...
            IViewExecutionOptions req = new ExecutionOptions(seq, flags, defaultExecutionOptions: new ViewCycleExecutionOptions(valuationTime, marketDataSpecifications), versionCorrection: new VersionCorrection(valuationTime, valuationTime));
            var runToCompletion = RunToCompletion(req);
            Assert.Equal(1, runToCompletion.Item1.Count());
            Assert.Equal(1, runToCompletion.Item2.Count());
            AssertApproximatelyEqual(req.ExecutionSequence.Next.ValuationTime, runToCompletion.Item2.Single().FullResult.ValuationTime);
        ...
        public Tuple<IEnumerable<ViewDefinitionCompiledArgs>, IEnumerable<CycleCompletedArgs>> RunToCompletion(IViewExecutionOptions options)
        {
            using (var remoteViewClient = Context.ViewProcessor.CreateViewClient())
            {
                ...
                using (var manualResetEvent = new ManualResetEvent(false))
                {
                    ...
                    remoteViewClient.AttachToViewProcess(GetViewDefinitionId(ViewName), options);
