...
{
    public class RemoteViewCycleTests : ViewTestBase
    {
        [Xunit.Extensions.Fact]
        public void CanGetCycle()
        {
           WithViewCycle(delegate(ViewDefinitionCompiledArgs compiled, IViewCycle cycle, RemoteViewClient client)
           {
        ...
        {
            WithViewCycle((compiled, cycle, client) =>
                          Assert.Throws<ArgumentException>(
                              () => cycle.QueryComputationCaches(new ComputationCacheQuery("Default"))),
                          Fixture.EquityViewDefinition.UniqueId);
        }
        ...
        public void CanHoldLotsOfCycles()
        {
            using (var remoteViewClient = Context.ViewProcessor.CreateViewClient())
            {
                ...
                listener.CycleCompleted += (sender, e) =>
                    {
                        ...
                        Parallel.For(
                            0, 
                            cyclesCount, 
                            _ => cycles.Add(remoteViewClient.CreateCycleReference(e.FullResult.ViewCycleId)));
                    };
                ...
                remoteViewClient.AttachToViewProcess(GetViewDefinitionId("Demo Equity Option Test View"), options);
        ...
        public void NumberOfResultsIsConsistent()
        {
            ...
            using (var mre = new ManualResetEvent(false))
            {
                ...
                var marketDataSpecifications = new List<MarketDataSpecification> {ExecutionOptions.GetDefaultMarketDataSpec()};
                var options = new ExecutionOptions(sequence, ViewExecutionFlags.TriggersEnabled | ViewExecutionFlags.AwaitMarketData, null, new ViewCycleExecutionOptions(default(DateTimeOffset), marketDataSpecifications));
                ...
                if (inconsistent.Any())
                {
                    ...
                    throw new Exception(string.Format("Inconsistent number of results for {0} {1}: {2}", Fixture.EquityViewDefinition.Name, counts, inconsistentString));
                }
            }
        }
        ...
        [Xunit.Extensions.Fact]
        public void CanGetAllTerminalValues()
        {
            WithViewCycle(
            delegate(ViewDefinitionCompiledArgs compiled, IViewCycle cycle, RemoteViewClient client)
            {
        ...
        {
            var targetMatches = n.Target.UniqueId == specToTest.TargetSpecification.UniqueId && n.Target.Type == specToTest.TargetSpecification.Type;
        ...
        public static void WithViewCycle(Action<ViewDefinitionCompiledArgs, IViewCycle, RemoteViewClient> action, UniqueId viewDefinitionId)
        {
            ...
            using (var remoteViewClient = Context.ViewProcessor.CreateViewClient())
            {
                ...
                listener.ViewDefinitionCompilationFailed +=
                    delegate(object sender, ViewDefinitionCompilationFailedArgs e)
                        {
                            error = e.Exception;
                            executedMre.Set();
                        };
                ...
                remoteViewClient.SetResultListener(listener);
                remoteViewClient.SetViewCycleAccessSupported(true);
                remoteViewClient.AttachToViewProcess(viewDefinitionId, ExecutionOptions.SingleCycle);
