...
namespace OpenGamma.Xunit.Extensions
{
    internal static class ExceptionUtility 
    {
        public static Exception GetExceptionWithStackTrace(Exception inner, StackTrace timedOutTrace)
        {
            ...
            (typeof(Exception).GetField("_remoteStackTraceString", BindingFlags.NonPublic | BindingFlags.Instance) ?? typeof(Exception).GetField("remote_stack_trace", BindingFlags.NonPublic | BindingFlags.Instance)).SetValue(inner, timedOutTrace + "$$RethrowMarker$$");
            return inner;
        }
